...
#### Crop & Threshold：消除全局干扰

线性组合产出的 Mask 有一个容易被忽略的问题：**它是全局的**。Proto Head 为全图生成了 32 张 Prototype Mask，而线性组合 $\sum \alpha_i P_i$ 本质上是这些全局模板的加权叠加。32 个系数不可能做到 100% 精确地"滤掉"其他目标的形状信息——目标 A 的 mask 边缘可能会残留目标 B 的一小片激活区域。这在分割结果中表现为**假阳性**（不属于任何目标的像素被错误标记为前景）。

YOLOv11 用两步后处理干净地解决了这个问题：

**第一步：Crop（裁剪）**

利用检测分支已经精确预测出的 Bounding Box，将每个实例的全局 Mask **强制裁剪到 BBox 范围内**——BBox 外部的所有像素值直接置零。

这个操作的物理含义很清晰：检测头已经精确框出了目标的位置，那 mask 就不需要操心"框外有没有东西"。砍掉框外的一切，就从根本上杜绝了 A 目标的 mask "误伤"到 B 目标区域的可能性。

> Crop 是检测和分割两个分支的一次巧妙协作——检测提供精准的空间边界，分割专注边界内部的轮廓刻画。两者各司其职带来的冗余，恰好用来消除彼此的误差。

**第二步：Threshold（阈值化）**

裁剪后的 Mask 首先通过双线性插值上采样到原图分辨率，然后经过一个简单的阈值判断：

$$\text{Mask}_{final}(x, y) = \begin{cases} 1 & \text{if } M_k(x, y) > 0.5 \\ 0 & \text{otherwise} \end{cases}$$

大于 0.5 的像素判为前景（属于该实例），小于等于 0.5 的判为背景。0.5 是 sigmoid 的自然分界点——当网络"拿不准"某个像素属于前景还是背景时，输出值接近 0.5；超过 0.5 意味着网络有足够的信心认为该像素属于这个实例。

两步下来，最终输出的是每个实例在原图上的干净、无干扰的二值 mask。
