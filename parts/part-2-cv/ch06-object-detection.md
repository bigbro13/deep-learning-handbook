...
```mermaid
flowchart TD
    A["所有预测框<br/>按置信度降序排列"] --> B["取出 top-1"]
    B --> C["计算当前 Precision & Recall"]
    C --> D{"还有框？"}
    D -->|"是"| E["加入下一个框"]
    E --> C
    D -->|"否"| F["得到 P-R 曲线上的<br/>一系列点"]
```
...
```mermaid
flowchart LR
    IN[输入特征] --> C1["Conv 1×1<br>降维到一半通道"]
    IN --> C2["Conv 1×1<br>降维到一半通道"]
    C1 --> BT["Bottleneck × N<br>3×3 卷积残差块"]
    C2 --> CAT[Concat<br>拼接两条分支]
    BT --> CAT
    CAT --> C3["Conv 1×1<br>融合 + 恢复通道数"]
    C3 --> OUT[输出特征]
```
...
```mermaid
flowchart LR
    N["YOLOv5n<br/>深度×0.33<br/>宽度×0.25<br/>~1.9M 参数"]
    S["YOLOv5s<br/>深度×0.33<br/>宽度×0.50<br/>~7.2M 参数"]
    M["YOLOv5m<br/>深度×0.67<br/>宽度×0.75<br/>~21.2M 参数"]
    L["YOLOv5l<br/>深度×1.0<br/>宽度×1.0<br/>~46.5M 参数"]
    X["YOLOv5x<br/>深度×1.33<br/>宽度×1.25<br/>~86.7M 参数"]
    N --> S --> M --> L --> X
```
...
flowchart TD
    subgraph 传统回归
        TRAD["网络直接输出<br/>宽度 = 203.0"]
    end
    subgraph DFL["DFL: 分布预测"]
        DIST["网络输出一个概率分布<br/>覆盖可能的宽度范围"]
        PROBS["P(w=200)=0.15<br/>P(w=201)=0.25<br/>P(w=202)=0.35<br/>P(w=203)=0.15<br/>P(w=204)=0.10"]
        FUS["加权求和 → 最终预测值<br/>= 200×0.15 + 201×0.25 + ... = 201.8"]
        DIST --> PROBS --> FUS
    end
    TRAD -.- DFL
```
...
flowchart LR
    subgraph C2f["C2f 模块（v8）"]
        direction LR
        C2F_IN[输入] --> C2F_SP[Split]
        C2F_SP --> C2F_B1["Bottleneck"]
        C2F_SP --> C2F_B2["Bottleneck"]
        C2F_SP --> C2F_B3["...N个"]
        C2F_SP --> C2F_SKIP["Shortcut"]
        C2F_B1 --> C2F_CAT[Concat]
        C2F_B2 --> C2F_CAT
        C2F_B3 --> C2F_CAT
        C2F_SKIP --> C2F_CAT
        C2F_CAT --> C2F_OUT[输出]
    end
    subgraph C3k2["C3k2 模块（v11）"]
        direction LR
        C3K_IN[输入] --> C3K_SP1["Split 1"]
        C3K_SP1 --> C3K_B1["Bottleneck × N"]
        C3K_SP1 --> C3K_SKIP1["Shortcut"]
        C3K_B1 --> C3K_SP2["Split 2"]
        C3K_SKIP1 --> C3K_CAT[Concat]
        C3K_SP2 --> C3K_C[Conv]
        C3K_SP2 --> C3K_B2["Bottleneck × N"]
        C3K_B2 --> C3K_CAT
        C3K_C --> C3K_CAT
        C3K_CAT --> C3K_OUT[输出]
    end
...
flowchart TD
    subgraph 传统检测流程
        T1[网络推理] --> T2[输出N个候选框] --> T3[NMS 筛选] --> T4[最终结果]
    end
    subgraph YOLOv26流程
        O1[网络推理] --> O2[直接输出最优结果<br>无后处理]
    end
    T3 -.-|"消除"| O2
```
