...
> 注意：TN 在这个例子里是 50，但在公式中一般不出现。因为"正确的拒绝"太多了（图片里大部分区域都是背景），统计它没有意义。
...
flowchart TD
    ...
    subgraph Backbone["Backbone: CSPDarknet + Focus"]
        FOCUS["Focus 切片层<br>（v6.0后替换为普通卷积）"]
        C3_1["C3 × 3<br>（128→256→512 通道）"]
        SPPF["SPPF<br>空间金字塔池化-快速版"]
    end
...
```mermaid
flowchart TD
    INPUT[输入图像] --> BACKBONE["共享 Backbone<br/>C3k2 + SPPF"]
    BACKBONE --> NECK["共享 Neck<br/>PAN-FPN"]
    NECK --> DET_HEAD["检测 Head<br/>分类分支 + 回归分支"]
    NECK --> SEG_HEAD["分割 Head<br/>掩膜原型 + 掩膜系数"]
    NECK --> POSE_HEAD["姿态 Head<br/>关键点坐标 + 可见性"]
    NECK --> OBB_HEAD["OBB Head<br/>5参数 + 角度回归"]

    DET_HEAD --> DET_OUT["类别 + 边界框<br/>4个坐标值"]
    SEG_HEAD --> SEG_OUT["类别 + 边界框<br/>+ 二值掩膜"]
    POSE_HEAD --> POSE_OUT["类别 + 边界框<br/>+ 17个关键点"]
    OBB_HEAD --> OBB_OUT["类别 + 旋转框<br/>(cx,cy,w,h,angle)"]
```
